...
- **默认只收 https 公网地址**，内网/环回/链路本地/CGNAT/基准测试段、裸主机名、IPv6 字面量、
  内嵌凭据的 URL 一律拒绝，且**不跟随任何重定向**（防校验被跳转绕过与 https→http 降级）；
- 校验发生在**发请求之前**，被拒的地址不会发出请求；
- **已知边界（明说）**：这是基于**主机名字符串**的检查，管不了 DNS 解析结果——把 A 记录指向
  `127.0.0.1`/内网的公网域名（nip.io 一类）仍会通过。扩展拿不到解析后 IP，是客观限制，
  所以只填**你自己信任的**分析源；
...
- 除上述四处外**不向任何第三方发送数据**，无遥测、无统计、无指纹；
- 但要说清：拉取叙事/推文会向 DeBot 与 FxTwitter 发查询，**它们的服务器能看到「某 IP 查了某代币」**——
  这是拉取式工具绕不开的，不是我们在采集（我们没有服务器也收不到）。观点/持仓者两页零网络，不受此影响；
