...
| 标签 | 内容 | 口径 |
| --- | --- | --- |
| **叙事**（默认） | DeBot 叙事（起源、评级理由、传播、开发者，中/EN 可切）+ 叙事来源推文 | DeBot 段内容全量；**起源/评级理由默认展开，传播/开发者/来源推文默认收起**（点一下就开） |
...
DeBot 的 1–5 星评分是主观打分，卡片**不显示星级**，避免被当成投资建议。
**评级理由那段文字保留**——理由是有用的上下文，分数不是。
...
这个扩展**没有后端**：没有服务器代码，没有账号体系，没有遥测。它一共只碰两个地方：
...
观点与持仓者两页**完全不联网**，只读页面上 fomo 已渲染的内容。

**要诚实说清**：你每看一个币，CA 会发给 DeBot、推文 ID 会发给 FxTwitter，
所以这两个第三方服务器能在自己日志里看到「某 IP 在某时刻查了某代币」。
这是拉取式工具绕不开的，但那不是我们在采集（我们没有服务器也收不到）。
介意的话只用观点/持仓者两页，那两页零网络。

**它做不到的事**：不读 fomo 登录态（不发任何请求给 fomo，不读取/存储/外发任何令牌）；
不碰钱包；不注入 `fomo.family` 以外的任何站点；不申请通配域名权限；不采集浏览行为。

**公开版不含「自定义分析源」**：那个能力要求扩展替你访问任意地址，公开发布即 SSRF 入口，
因此设置面板、请求路径与通配权限已整体移除。完整说明见 [PRIVACY.md](PRIVACY.md)。

其它不变式：

- 缓存分档：DeBot 10 分钟、推文 30 分钟，点「↻」强制重拉；
...
background.js   service worker：代发 DeBot / FxTwitter 请求 + 分档缓存
...
测试用 fixture 桩掉 DeBot 与推文接口，在真实 Chromium 里跑完整渲染链路并逐步截图。
...
**换币立刻清场（轮询与点击两条路都不残留上一只币）** / 空状态 / 页脚链接排除 / 钱包地址陷阱 / fiber 解析 / **「AI 判断」段整段不存在** /
**设置面板只剩通用组（分析源输入框/高级开关/保存钮全部不存在）** /
**manifest 零通配授权 + permissions 只有 storage + 固定公钥在位** /
