...
const versionParts=JSON.parse(fs.readFileSync(EXT+'/manifest.json')).version.split('.').map(Number);
versionParts[versionParts.length-1]++;
const NEXT_VERSION=versionParts.join('.');
...
const server=https.createServer({key:fs.readFileSync(temp+'/key'),cert:fs.readFileSync(temp+'/cert')},(req,res)=>{
 ...
 if(u.hostname==='api.github.com')return reply({tag_name:'v'+NEXT_VERSION,html_url:'https://github.com/mickeyhogen/fomo-helper/releases/tag/v'+NEXT_VERSION,body:'- Next stable release for testing.'});
...
try{
 ...
 check('upgrade keeps the fixed extension ID and loads the new package',now.id===original.id&&now.manifest.version===JSON.parse(fs.readFileSync(EXT+'/manifest.json')).version);
 ...
 check('enabling updates shows the newer stable release in the card',!!await until(()=>shadow(p,`const a=this.querySelector('.upd');return a&&a.textContent.includes('v${NEXT_VERSION}')&&a.href==='https://github.com/mickeyhogen/fomo-helper/releases/tag/v${NEXT_VERSION}';`)));
 await popup.select('#lang','zh');
 check('popup switches to Chinese and retains the version',await popup.evaluate(()=>document.documentElement.lang==='zh-CN'&&document.querySelector('[data-i18n="edition"]').textContent.includes('检查更新')&&document.querySelector('#bver').textContent.includes(chrome.runtime.getManifest().version)));
